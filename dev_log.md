...
### 実装ログ（このセッション）
- [x] `MinionCatalog.cs` 新規（純staticデータ土台・既存シーン/コード非依存＝MCP無しでも安全）：配下ロスター16種＝3ファミリー(不死/獣/魔族)×5役割(盾/近接/遠隔/支援/妨害)。原作CPティア準拠(ラット1/バット2/ウルフ3/ゾンビ4/ゴブリン5/スケルトン…/コボルト10/大獣10/インプ15/オーク20/ダークエルフ50)。各Def=family/role/tierCP/hp・atk・spd倍率/rig(ファミリーリグ流用)/AttackStyle/spumHint(後でSPUM/Dungeon Taleへ差替の当たり)/note。FamilyTrait(不死=とどめ再生成/獣=加速/魔族=吸血)はデータのみ(挙動化は後)。ByFamily/ByRole/Get/TryGet/RoleName等の参照ヘルパ。
- 未（MCP再接続後）: ①配線=DungeonFeatureManager.SelectedSpecies→SelectedMinion(MinionType)に拡張しSpawnDefenderでDef適用、ZombieAIにrole/minionId保持 ②UI=下部バーの眷属セレクタを「図鑑(ファミリー→役割→個体)」に拡張 ③Bloodlines UIフレームワーク(黒×赤スキン+拡張パネル) ④部屋スロット編成 ⑤種族機械的個性の実挙動化。
- 注意: Unity MCP はこのセッションで切断。コンパイル確認/シーン配線/検証は再接続後。MinionCatalogは他ファイル未改変ゆえ単体で通る想定。

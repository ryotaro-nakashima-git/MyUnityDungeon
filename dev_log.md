...
### 次アクション候補
- Step 2A動作確認OK → **Step 2B（複数フロア/階層）** へ、または **トラック2 A案（種族進化＋ツリー）**。
- UI発展：Step 3の「主要要素の手動配置モード」（トーテム/スポナー/ボス/特殊敵）＝下部ツールの近日ボタンを実装。研究ツリー画面。角丸スプライト等の見た目仕上げ。
- ダンジョン表示のカメラ自動フィット（生成サイズに合わせてズーム）。
